...
from models.d_nerf.run_dnerf_helpers import *
...
def batchify(fn, chunk):
    ...
    def ret(inputs_pos, inputs_time):
        ...
        for i in range(0, num_batches, chunk):
            out, dx = fn(
                inputs_pos[i : i + chunk],
                [inputs_time[0][i : i + chunk], inputs_time[1][i : i + chunk]],
            )
            out_list += [out]
            dx_list += [dx]
...
def run_network(
    ...
    position_delta = torch.reshape(
        position_delta_flat, list(inputs.shape[:-1]) + [position_delta_flat.shape[-1]]
    )
...
def render_rays(
    ...
    **kwargs,
    ...
    if z_vals is None:
        t_vals = torch.linspace(0.0, 1.0, steps=N_samples).to(far.device)
        if not lindisp:
...
def config_parser():
    ...
    # training options
    parser.add_argument(
        "--nerf_type", type=str, default="original", help="DNeRF network type"
    )
    parser.add_argument(
        "--N_iter", type=int, default=500000, help="num training iterations"
    )
    ...
    parser.add_argument(
        "--tv_loss_weight", type=float, default=1.0e-4, help="weight of tv loss"
    )
...
def train():
    ...
    assert min_time == 0.0, "time must start at 0"
    assert max_time == 1.0, "max time must be 1"
    ...
    for i in trange(start, N_iters):
        ...
        if args.add_tv_loss:
            ...
            if frame_time_prev is not None:
                rand_time_prev = (
                    frame_time_prev + (frame_time - frame_time_prev) * torch.rand(1)[0]
                )
                _, _, _, extras_prev = render(
                    H,
                    W,
                    focal,
                    chunk=args.chunk,
                    rays=batch_rays,
                    frame_time=rand_time_prev,
                    verbose=i < 10,
                    retraw=True,
                    z_vals=extras["z_vals"].detach(),
                    **render_kwargs_train,
                )

            if frame_time_next is not None:
                rand_time_next = (
                    frame_time + (frame_time_next - frame_time) * torch.rand(1)[0]
                )
                _, _, _, extras_next = render(
                    H,
                    W,
                    focal,
                    chunk=args.chunk,
                    rays=batch_rays,
                    frame_time=rand_time_next,
                    verbose=i < 10,
                    retraw=True,
                    z_vals=extras["z_vals"].detach(),
                    **render_kwargs_train,
        ...
        if args.add_tv_loss:
            if frame_time_prev is not None:
                tv_loss += (
                    (extras["position_delta"] - extras_prev["position_delta"]).pow(2)
                ).sum()
                if "position_delta_0" in extras:
                    tv_loss += (
                        (
                            extras["position_delta_0"] - extras_prev["position_delta_0"]
                        ).pow(2)
                    ).sum()
            if frame_time_next is not None:
                tv_loss += (
                    (extras["position_delta"] - extras_next["position_delta"]).pow(2)
                ).sum()
                if "position_delta_0" in extras:
                    tv_loss += (
                        (
                            extras["position_delta_0"] - extras_next["position_delta_0"]
                        ).pow(2)
                    ).sum()
        ...
        if i % args.i_print == 0:
            tqdm_txt = (
                f"[TRAIN] Iter: {i} Loss_fine: {img_loss.item()} PSNR: {psnr.item()}"
            )
            if args.add_tv_loss:
        ...
        if i % args.i_testset == 0:
            ...
            with torch.no_grad():
                render_path(
                    ...
                    torch.Tensor(times[i_test]).to(device),
                    hwf,
                    args.chunk,
                    render_kwargs_test,
