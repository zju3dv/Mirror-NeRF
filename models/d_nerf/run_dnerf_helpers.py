...
def get_embedder(multires, input_dims, i=0):
    ...
    embedder_obj = Embedder_DNeRF(**embed_kwargs)
...
class DirectTemporalNeRF(nn.Module):
    ...
    def forward(self, x, ts):
        ...
        assert (
            len(torch.unique(t[:, :1])) == 1
        ), "Only accepts all points from same time"
        cur_time = t[0, 0]
...
class DNeRF:
    ...
    def get_by_name(type, *args, **kwargs):
        print("DNeRF type selected: %s" % type)
...
class NeRFOriginal(nn.Module):
    def __init__(
        ...
        ### Implementation according to the official code release (https://github.com/bmild/DNeRF/blob/master/run_nerf_helpers.py#L104-L105)
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    if det:
        u = torch.linspace(0.0, 1.0, steps=N_samples).to(cdf.device)
